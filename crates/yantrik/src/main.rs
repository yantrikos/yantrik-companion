...
enum Commands {
    ...
    /// Start MCP (Model Context Protocol) stdio server for YantrikDB.
    Mcp {
        /// Path to memory database.
        #[arg(long, default_value = "memory.db")]
        db: String,
        ...
        embedding_dim: usize,
        /// Path to pre-downloaded embedder model directory.
        #[arg(long)]
...
fn main() {
    ...
    // MCP subcommand needs tracing on stderr (stdout is JSON-RPC).
    // Handle it before the default tracing init.
    if let Commands::Mcp { db, embedding_dim, embedder_dir } = cli.command {
        cmd_mcp(db, embedding_dim, embedder_dir);
        return;
    }

    // Initialize tracing (stdout) for all other subcommands
...
fn cmd_serve(config_path: Option<PathBuf>) {
    ...
    // Start chat bridge (Telegram, etc.) if any providers are enabled
    let chat_state = state.clone();
    let chat_state2 = state.clone();
    let _chat_handle = {
        let companion_guard = chat_state.service.lock().unwrap();
        let cfg = companion_guard.config.clone();
        drop(companion_guard);

        let ai_cb: yantrik_chat::router::AiCallback = Box::new(move |msg: &str, _history: &[String], _policy: &yantrik_chat::policy::ConversationPolicy| -> Option<String> {
            let mut companion = chat_state2.service.lock().unwrap();
            let resp = companion.handle_message(msg);
            Some(resp.message)
        });

        let brain_cb: yantrik_chat::router::BrainCallback = Box::new(|_sender, _sender_id, _provider, _content_type| {
            // Brain feeding can be added later
        });

        yantrik_companion::chat_bridge::start_chat(&cfg, ai_cb, brain_cb)
    };
    if _chat_handle.is_some() {
        tracing::info!("Chat bridge started (Telegram/messaging providers active)");
    }
    ...
    let think_handle = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            while think_running_clone.load(Ordering::Relaxed) {
                ...
                let mut companion = match think_state.service.try_lock() {
                    Ok(guard) => guard,
                    Err(std::sync::TryLockError::WouldBlock) => {
                        tracing::info!("Think cycle skipped: service busy (HTTP request in progress)");
                        continue;
                    }
                    Err(e) => {
...
fn cmd_mcp(db: String, embedding_dim: usize, embedder_dir: Option<PathBuf>) {
    // MCP stdio uses stdout for JSON-RPC — redirect tracing to stderr
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new("info")),
    ...
    rt.block_on(async {
        if let Err(e) = yantrik_mcp::run_server(&db, embedding_dim, embedder_dir.as_deref()).await {
