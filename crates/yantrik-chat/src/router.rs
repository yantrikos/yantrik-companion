...
use crate::manager::OutboundRequest;
use crate::model::*;
...
pub struct ChatRouter {
    ...
    /// Outbound channels per provider (send replies through provider threads).
...
impl ChatRouter {
    ...
    /// Wire outbound channels from the manager (call after all providers started).
    pub fn set_outbound_senders(&mut self, senders: HashMap<String, Sender<OutboundRequest>>) {
        self.outbound_txs = senders;
    }
    ...
    fn handle_event(&self, provider_id: &str, event: InboundEvent) {
        ...
        if should_reply {
            if let Some(ai_cb) = &self.ai_callback {
                ...
                // Send 👀 reaction + typing indicator before AI processing starts
                if let Some(outbound_tx) = self.outbound_txs.get(provider_id) {
                    let _ = outbound_tx.send(OutboundRequest::Reaction {
                        message: msg.message.clone(),
                        emoji: "\u{1F440}".to_string(), // 👀
                    });
                    let _ = outbound_tx.send(OutboundRequest::Typing {
                ...
                if let Some(response) = ai_cb(&content_text, &context, &policy) {
                    // Send response through provider's outbound channel
                    let out_msg = OutboundMessage::text(&response)
                        .with_reply(msg.message.clone());

                    if let Some(outbound_tx) = self.outbound_txs.get(provider_id) {
                        let (result_tx, result_rx) = crossbeam_channel::bounded(1);
                        let req = OutboundRequest::Send {
                            conversation: msg.conversation.clone(),
                            message: out_msg,
                            result_tx,
                        };

                        if outbound_tx.send(req).is_ok() {
                            // Wait for provider thread to process the send (up to 30s)
                        ...
                        tracing::warn!(
                            provider = provider_id,
                            "No outbound channel for provider"
                        );
                    }
