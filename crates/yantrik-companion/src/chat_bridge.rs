...
pub fn start_chat(
    ...
    // Native mobile WebSocket
    if chat.native.enabled && !chat.native.auth_tokens.is_empty() {
        let provider = yantrik_chat::providers::native::NativeProvider::new(
            chat.native.port,
            chat.native.auth_tokens.clone(),
            chat.native.max_clients,
        );
        manager.start_provider(Box::new(provider));
        started += 1;
    }
    ...
    // Wire outbound channels from manager into router (for sending replies)
    router.set_outbound_senders(manager.outbound_senders());
