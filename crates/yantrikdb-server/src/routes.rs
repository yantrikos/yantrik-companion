...
pub fn routes() -> Router<AppStateRef> {
    Router::new()
        ...
        // Memory CRUD endpoints (for ZeroClaw YantrikDB backend)
        .route("/memory/store", post(memory_store))
        .route("/memory/recall", post(memory_recall))
        .route("/memory/list", get(memory_list))
        .route("/memory/count", get(memory_count))
        .route("/memory/{key}", get(memory_get))
...
async fn chat(
    ...
    let result = tokio::task::spawn_blocking(move || {
        // Retry try_lock for up to 30s to avoid blocking indefinitely
        // when the think cycle holds the mutex for EXECUTE urge LLM calls.
        let mut service = {
            let mut guard = None;
            for _ in 0..60 {
                match state.service.try_lock() {
                    Ok(s) => { guard = Some(s); break; }
            ...
            match guard {
                Some(s) => s,
                None => {
                    tracing::warn!("chat: service busy after 30s, returning 503");
...
async fn memory_store(
    ...
    // Map ZeroClaw category to YantrikDB memory_type.
    let memory_type = match req.category.as_str() {
        "core" => "fact",
        "daily" => "episode",
    ...
    let importance = match req.category.as_str() {
        "core" => 0.8,
        "daily" => 0.5,
        "conversation" => 0.4,
        _ => 0.6,
    ...
    match service.db.record_text(
        &req.content,
        memory_type,
        importance,
        0.0,        // neutral valence
        168.0,      // 7-day half-life
        &serde_json::json!({ "key": req.key, "session_id": req.session_id }),
        "default",
        0.9,
        "general",
        "zeroclaw",
        ...
        Ok(rid) => Ok(Json(serde_json::json!({ "rid": rid, "status": "stored" }))),
        Err(e) => {
            tracing::warn!("memory_store failed: {e}");
...
async fn memory_recall(
    ...
    let results = service
        .db
        .recall_text(&req.query, req.limit)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn memory_get(
    ...
    // Search for the memory by key (use recall with the key as query).
    let results = service
        .db
        .recall_text(&key, 1)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn memory_list(
    State(state): State<AppStateRef>,
    axum::extract::Query(query): axum::extract::Query<MemoryListQuery>,
) -> Result<Json<Vec<serde_json::Value>>, StatusCode> {
    let service = state
        .service
        .lock()
    ...
    // Filter by domain if category is specified.
    let domain_filter = query.category.as_deref();
    let source_filter = None;

    let results = service
        .db
        .recall_text_filtered("*", 100, domain_filter, source_filter)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn memory_forget(
    ...
    match service.db.forget(&key) {
        Ok(deleted) => Ok(Json(serde_json::json!({ "deleted": deleted }))),
        Err(e) => {
            tracing::warn!("memory_forget failed: {e}");
            Err(StatusCode::INTERNAL_SERVER_ERROR)
        }
    }
}
...
async fn memory_count(
    ...
    match service.db.stats(None) {
        Ok(s) => Ok(Json(serde_json::json!({ "count": s.active_memories }))),
        Err(_) => Ok(Json(serde_json::json!({ "count": 0 }))),
    }
}
...
async fn oai_stream(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        service.handle_message_streaming(&user_text, |token| {
            ...
            // __REPLACE__ is a CLI streaming display artifact (overwrites
            // previous line in terminal).  Strip it for HTTP consumers —
            // the next token is the replacement text and will be sent
            // normally.
